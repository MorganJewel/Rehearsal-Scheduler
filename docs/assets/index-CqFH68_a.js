(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const l of document.querySelectorAll('link[rel="modulepreload"]'))o(l);new MutationObserver(l=>{for(const a of l)if(a.type==="childList")for(const s of a.addedNodes)s.tagName==="LINK"&&s.rel==="modulepreload"&&o(s)}).observe(document,{childList:!0,subtree:!0});function n(l){const a={};return l.integrity&&(a.integrity=l.integrity),l.referrerPolicy&&(a.referrerPolicy=l.referrerPolicy),l.crossOrigin==="use-credentials"?a.credentials="include":l.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function o(l){if(l.ep)return;l.ep=!0;const a=n(l);fetch(l.href,a)}})();const K=new Map;let D=null;function C(e,t){K.set(e,t)}function ae(e){D=e}function g(e){window.location.hash="#"+e}function le(e,t){const n=e==="/"?[]:e.split("/").filter(Boolean),o=t==="/"?[]:t.split("/").filter(Boolean);if(n.length!==o.length)return null;const l={};for(let a=0;a<n.length;a++)if(n[a].startsWith(":"))l[n[a].slice(1)]=decodeURIComponent(o[a]);else if(n[a]!==o[a])return null;return l}function z(){const t=(window.location.hash||"#/").slice(1)||"/";for(const[n,o]of K){const l=le(n,t);if(l!==null){o(l);return}}D&&D()}function se(){window.addEventListener("hashchange",z),z()}function H(){return crypto.randomUUID()}let $=[{id:"demo-prod-1",name:"Hamlet",playwright:"William Shakespeare",created_at:new Date().toISOString()}],v=[{id:"demo-sched-1",production_id:"demo-prod-1",name:"Act I Run-Through",date:"2026-06-15",created_at:new Date().toISOString()}],y=[{id:"demo-block-1",schedule_id:"demo-sched-1",scene_name:"Act I, Scene 1 — The Battlements",actors:"Hamlet, Horatio, Marcellus, Ghost",block_type:"blocking",duration_minutes:40,notes:`Work Ghost entrance from SR. Hamlet crosses to DSL on "I'll follow it."`,order_index:0},{id:"demo-block-2",schedule_id:"demo-sched-1",scene_name:"Act I, Scene 2 — The Court",actors:"Hamlet, Claudius, Gertrude, Polonius",block_type:"table work",duration_minutes:30,notes:`Discuss Hamlet's internal conflict. Emphasis on the "too too solid flesh" speech.`,order_index:1},{id:"demo-block-3",schedule_id:"demo-sched-1",scene_name:"Act I, Scene 3 — Polonius's House",actors:"Laertes, Ophelia, Polonius",block_type:"run-through",duration_minutes:25,notes:"Focus on Ophelia's reaction to advice. Speed run first, then notes.",order_index:2}];function ie(){return[...$].sort((e,t)=>t.created_at.localeCompare(e.created_at))}function N(e){return $.find(t=>t.id===e)??null}function de({name:e,playwright:t}){const n={id:H(),name:e,playwright:t||null,created_at:new Date().toISOString()};return $.push(n),n}function V(e,{name:t,playwright:n}){const o=$.find(l=>l.id===e);return o?(o.name=t,o.playwright=n||null,o):null}function J(e){v.filter(n=>n.production_id===e).map(n=>n.id).forEach(n=>O(n)),$=$.filter(n=>n.id!==e)}function re(e){return v.filter(t=>t.production_id===e).sort((t,n)=>t.created_at.localeCompare(n.created_at))}function Q(e){return v.find(t=>t.id===e)??null}function ce({name:e,date:t,production_id:n}){const o={id:H(),name:e,date:t||null,production_id:n,created_at:new Date().toISOString()};return v.push(o),o}function q(e,{name:t,date:n}){const o=v.find(l=>l.id===e);return o?(o.name=t,o.date=n||null,o):null}function O(e){y=y.filter(t=>t.schedule_id!==e),v=v.filter(t=>t.id!==e)}function ue(e){return y.filter(t=>t.schedule_id===e).sort((t,n)=>t.order_index-n.order_index)}function me({schedule_id:e,scene_name:t,actors:n,block_type:o,duration_minutes:l,notes:a,order_index:s}){const i={id:H(),schedule_id:e,scene_name:t||null,actors:n||null,block_type:o||null,duration_minutes:l||null,notes:a||null,order_index:s??0};return y.push(i),i}function pe(e,t){const n=y.find(o=>o.id===e);return n?(Object.assign(n,{scene_name:t.scene_name||null,actors:t.actors||null,block_type:t.block_type||null,duration_minutes:t.duration_minutes||null,notes:t.notes||null,order_index:t.order_index??n.order_index}),n):null}function he(e){y=y.filter(t=>t.id!==e)}const E=[{title:"Welcome to Rehearsal Scheduler",text:"This quick tour walks you through every feature — takes about 2 minutes. Hit Next to begin, or Skip to jump straight in.",target:null,page:null},{title:"Your Dashboard",text:"The Dashboard lists all your productions — plays, musicals, readings, anything you're directing. We've pre-loaded a demo production so you can explore right away.",target:"#productions-list",page:"/"},{title:"Create a Production",text:'Click "+ New Production" to start a show. Give it a title and an optional playwright credit. Productions are the top level of everything.',target:"#new-production-btn",page:"/"},{title:"Production Page",text:"Clicking a production card opens its detail page. The title and playwright sit at the top, and all rehearsal schedules for that show are listed below.",target:".page-title",page:"/production/demo-prod-1"},{title:"Schedules",text:'A schedule is one rehearsal session — usually one per day. Click a schedule card to open it, or use "+ New Schedule" to add one. Each schedule belongs to exactly one production.',target:"#schedules-list",page:"/production/demo-prod-1"},{title:"Schedule View",text:"Opening a schedule shows all its blocks in order. The total duration in minutes is summed up automatically next to the Blocks heading.",target:"#blocks-list",page:"/schedule/demo-sched-1"},{title:"Rehearsal Blocks",text:"Each card is one block: a scene, a run-through, table work, a tech call — whatever you need. Blocks show scene name, actors, type badge, duration, and your staging notes.",target:".block-card",page:"/schedule/demo-sched-1"},{title:"Adding & Editing Blocks",text:'Click "+ Add Block" to create a new one. Hit Edit on any block to change it. Each block has: scene name, actors, block type, duration in minutes, order index, and notes.',target:"#add-block-btn",page:"/schedule/demo-sched-1"},{title:"Block Types",text:"Five types, each colour-coded: Blocking (blue), Run-through (green), Table Work (gold), Tech (purple), Other (grey). Use them to visually scan a schedule at a glance.",target:".type-badge",page:"/schedule/demo-sched-1"},{title:"AI Schedule Names",text:"The Apertus AI panel reads your blocks and suggests 3 creative schedule names tailored to your scenes and actors. Click any suggestion to instantly rename the schedule.",target:".ai-panel",page:"/schedule/demo-sched-1"},{title:"You're All Set!",text:"That covers everything. Go create your own productions, build schedules, fill in blocks, and try the AI tool. Everything resets on refresh — so experiment freely.",target:null,page:null}];let u=-1;function X(){u=0,P()}async function P(){if(te(),u<0||u>=E.length){A();return}const e=E[u];e.page&&(window.location.hash.slice(1)||"/")!==e.page&&(g(e.page),await Y(220));const t=e.target?document.querySelector(e.target):null;t?(t.scrollIntoView({behavior:"instant",block:"center"}),await Y(60),ge(t),fe(e,t)):be(e)}function ge(e){const n=e.getBoundingClientRect(),o=n.left-10,l=n.top-10,a=n.width+10*2,s=n.height+10*2,i=document.createElement("div");i.id="tour-overlay",i.style.cssText="position:fixed;inset:0;z-index:9000;pointer-events:none",i.innerHTML=`
  ...
  `}function I(){var e;(e=document.getElementById("tour-btn"))==null||e.addEventListener("click",X)}function m({title:e,bodyHTML:t,onSubmit:n,submitLabel:o="Save",showFooter:l=!0}){var s,i;r();const a=document.createElement("div");a.id="modal-overlay",a.className="modal-overlay",a.innerHTML=`
  ...
  `,I(),document.getElementById("new-production-btn").addEventListener("click",ye),_()}function _(){const e=document.getElementById("productions-list");if(!e)return;const t=ie();if(!t.length){e.innerHTML=`
    ...
    <div class="card card-clickable" data-id="${n.id}">
      <div class="card-header">
        <div>
          <div class="card-title">${M(n.name)}</div>
          ${n.playwright?`<div class="card-subtitle">by ${M(n.playwright)}</div>`:""}
        </div>
  ...
  `).join(""),e.querySelectorAll(".card-clickable").forEach(n=>{n.addEventListener("click",o=>{o.target.closest(".btn")||g("/production/"+n.dataset.id)})}),e.querySelectorAll(".edit-btn").forEach(n=>{n.addEventListener("click",o=>{o.stopPropagation(),ke(n.dataset.id,n.dataset.name,n.dataset.playwright)})}),e.querySelectorAll(".delete-btn").forEach(n=>{n.addEventListener("click",o=>{o.stopPropagation(),Ee(n.dataset.id,n.dataset.name)})})}function ne(e="",t=""){return`
  ...
  `}function ye(){m({title:"New Production",bodyHTML:ne(),submitLabel:"Create",onSubmit:()=>{const e=document.getElementById("prod-name").value.trim();if(!e){document.getElementById("prod-error").textContent="Name is required.";return}de({name:e,playwright:document.getElementById("prod-playwright").value.trim()}),r(),_()}})}function ke(e,t,n){m({title:"Edit Production",bodyHTML:ne(t,n),submitLabel:"Save",onSubmit:()=>{const o=document.getElementById("prod-name").value.trim();if(!o){document.getElementById("prod-error").textContent="Name is required.";return}V(e,{name:o,playwright:document.getElementById("prod-playwright").value.trim()}),r(),_()}})}function Ee(e,t){var n;m({title:"Delete Production",bodyHTML:`
      <p>Delete <strong>${M(t)}</strong>? All schedules and blocks will be removed.</p>
    `,submitLabel:"Delete",onSubmit:()=>{J(e),r(),_()}}),(n=document.getElementById("modal-submit-btn"))==null||n.classList.replace("btn-primary","btn-danger")}function M(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function B(e){return String(e??"").replace(/"/g,"&quot;")}function we(e){const t=N(e),n=document.getElementById("app");if(!t){n.innerHTML=`
      ${x()}
      <div class="page">
        <p class="error-msg">Production not found.</p>
        <a href="#/" class="btn btn-ghost" style="margin-top:1rem">← Dashboard</a>
      </div>
    `,I();return}L(e)}function L(e){const t=N(e);if(!t){g("/");return}const n=re(e),o=document.getElementById("app");o.innerHTML=`
    ${x()}
    <div class="page">
  ...
  `,I(),document.getElementById("edit-prod-btn").addEventListener("click",()=>Be(e,t,()=>L(e))),document.getElementById("delete-prod-btn").addEventListener("click",()=>xe(e,t.name)),document.getElementById("new-sched-btn").addEventListener("click",()=>Ie(e,()=>L(e))),Se(e,()=>L(e))}function $e(e){return e.length?e.map(t=>`
    <div class="card card-clickable" data-id="${t.id}">
      <div class="card-header">
        <div>
          <div class="card-title">${w(t.name)}</div>
          ${t.date?`<div class="card-subtitle">${Te(t.date)}</div>`:""}
        </div>
        <div class="card-actions">
  ...
  `).join(""):`
    ...
    `}function Se(e,t){document.querySelectorAll(".card-clickable").forEach(n=>{n.addEventListener("click",o=>{o.target.closest(".btn")||g("/schedule/"+n.dataset.id)})}),document.querySelectorAll(".sched-edit").forEach(n=>{n.addEventListener("click",o=>{o.stopPropagation(),Le(n.dataset.id,n.dataset.name,n.dataset.date,t)})}),document.querySelectorAll(".sched-del").forEach(n=>{n.addEventListener("click",o=>{o.stopPropagation(),_e(n.dataset.id,n.dataset.name,t)})})}function Be(e,t,n){m({title:"Edit Production",bodyHTML:`
      <div class="form-group">
      ...
      <p class="error-msg" id="prod-error"></p>
    `,submitLabel:"Save",onSubmit:()=>{const o=document.getElementById("prod-name").value.trim();if(!o){document.getElementById("prod-error").textContent="Name required.";return}V(e,{name:o,playwright:document.getElementById("prod-playwright").value.trim()}),r(),n()}})}function xe(e,t){var n;m({title:"Delete Production",bodyHTML:`<p>Delete <strong>${w(t)}</strong>? All schedules and blocks will be removed.</p>`,submitLabel:"Delete",onSubmit:()=>{J(e),r(),g("/")}}),(n=document.getElementById("modal-submit-btn"))==null||n.classList.replace("btn-primary","btn-danger")}function oe(e="",t=""){return`
    <div class="form-group">
  ...
  `}function Ie(e,t){m({title:"New Schedule",bodyHTML:oe(),submitLabel:"Create",onSubmit:()=>{const n=document.getElementById("sched-name").value.trim();if(!n){document.getElementById("sched-error").textContent="Name required.";return}ce({name:n,date:document.getElementById("sched-date").value||null,production_id:e}),r(),t()}})}function Le(e,t,n,o){m({title:"Edit Schedule",bodyHTML:oe(t,n),submitLabel:"Save",onSubmit:()=>{const l=document.getElementById("sched-name").value.trim();if(!l){document.getElementById("sched-error").textContent="Name required.";return}q(e,{name:l,date:document.getElementById("sched-date").value||null}),r(),o()}})}function _e(e,t,n){var o;m({title:"Delete Schedule",bodyHTML:`<p>Delete <strong>${w(t)}</strong>? All blocks will be removed.</p>`,submitLabel:"Delete",onSubmit:()=>{O(e),r(),n()}}),(o=document.getElementById("modal-submit-btn"))==null||o.classList.replace("btn-primary","btn-danger")}function Te(e){if(!e)return"";const[t,n,o]=e.split("-");return new Date(+t,+n-1,+o).toLocaleDateString("en-US",{weekday:"long",year:"numeric",month:"long",day:"numeric"})}function w(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function f(e){return String(e??"").replace(/"/g,"&quot;")}const De=[50,60,5,45,23,2,62,34,54,21,11,24,46,27,52,29,46,56,61,49,29,63,47,61,53,51,54,54,56,34,41,53,14,52,23,23,23],Pe=De.map(e=>String.fromCharCode(e^90)).join(""),Ae=["blocking","run-through","table work","tech","other"];function Me(e){const t=Q(e),n=document.getElementById("app");if(!t){n.innerHTML=`
      ${x()}
      <div class="page">
        <p class="error-msg">Schedule not found.</p>
        <a href="#/" class="btn btn-ghost" style="margin-top:1rem">← Dashboard</a>
      </div>
    `,I();return}k(e)}function k(e){const t=Q(e);if(!t){g("/");return}const n=N(t.production_id),o=ue(e),l=o.reduce((s,i)=>s+(i.duration_minutes||0),0),a=document.getElementById("app");a.innerHTML=`
    ${x()}
    <div class="page">
      <div class="breadcrumb">
        <a href="#/">Dashboard</a> /
      ...
      <div class="page-header">
        <div class="page-header-left">
          ...
          ${t.date?`<p class="page-subtitle">${Ue(t.date)}</p>`:""}
        </div>
        <div style="display:flex;gap:0.5rem">
  ...
  `,I(),document.getElementById("edit-sched-btn").addEventListener("click",()=>Oe(e,t,()=>k(e))),document.getElementById("delete-sched-btn").addEventListener("click",()=>Re(e,t.name,n==null?void 0:n.id)),document.getElementById("add-block-btn").addEventListener("click",()=>G(null,e,o.length,()=>k(e))),document.getElementById("ai-btn").addEventListener("click",()=>Fe(e,t,n,o)),document.querySelectorAll(".block-edit").forEach(s=>{s.addEventListener("click",()=>{const i=o.find(d=>d.id===s.dataset.id);i&&G(i,e,i.order_index,()=>k(e))})}),document.querySelectorAll(".block-del").forEach(s=>{s.addEventListener("click",()=>qe(s.dataset.id,s.dataset.name,()=>k(e)))})}function Ce(e){return e.length?e.map((t,n)=>`
    <div class="block-card">
      <div class="block-top">
        <div>
          ...
          ${t.block_type?`<span class="type-badge ${je(t.block_type)}" style="margin-left:0.6rem">${h(t.block_type)}</span>`:""}
        </div>
    ...
    <div class="form-row">
      <div class="form-group">
        ...
        <select class="form-select" id="b-type">
          ...
          ${Ae.map(t=>`<option value="${t}" ${e.block_type===t?"selected":""}>${t}</option>`).join("")}
        </select>
      </div>
      <div class="form-group">
  ...
  `}function Ne(e){return{schedule_id:e,scene_name:document.getElementById("b-scene").value.trim()||null,actors:document.getElementById("b-actors").value.trim()||null,block_type:document.getElementById("b-type").value||null,duration_minutes:parseInt(document.getElementById("b-duration").value)||null,order_index:parseInt(document.getElementById("b-order").value)||0,notes:document.getElementById("b-notes").value.trim()||null}}function G(e,t,n,o){const l=!!e;m({title:l?"Edit Block":"Add Block",bodyHTML:He(e??{order_index:n}),submitLabel:l?"Save":"Add Block",onSubmit:()=>{const a=Ne(t);l?pe(e.id,a):me(a),r(),o()}})}function qe(e,t,n){var o;m({title:"Delete Block",bodyHTML:`<p>Delete block <strong>${h(t)}</strong>?</p>`,submitLabel:"Delete",onSubmit:()=>{he(e),r(),n()}}),(o=document.getElementById("modal-submit-btn"))==null||o.classList.replace("btn-primary","btn-danger")}function Oe(e,t,n){m({title:"Edit Schedule",bodyHTML:`
      <div class="form-group">
        <label class="form-label">Name *</label>
        <input class="form-input" id="sched-name" type="text"
      ...
      <p class="error-msg" id="sched-error"></p>
    `,submitLabel:"Save",onSubmit:()=>{const o=document.getElementById("sched-name").value.trim();if(!o){document.getElementById("sched-error").textContent="Name required.";return}q(e,{name:o,date:document.getElementById("sched-date").value||null}),r(),n()}})}function Re(e,t,n){var o;m({title:"Delete Schedule",bodyHTML:`<p>Delete <strong>${h(t)}</strong>? All blocks will be removed.</p>`,submitLabel:"Delete",onSubmit:()=>{O(e),r(),g("/production/"+n)}}),(o=document.getElementById("modal-submit-btn"))==null||o.classList.replace("btn-primary","btn-danger")}async function Fe(e,t,n,o){var R,F,j;const l=Pe,a=document.getElementById("ai-status"),s=document.getElementById("ai-suggestions"),i=document.getElementById("ai-btn");i.disabled=!0,a.style.display="block",a.textContent="Thinking…",s.innerHTML="";const d=o.length?o.map(p=>`- ${p.scene_name||"Untitled"} | actors: ${p.actors||"none"} | type: ${p.block_type||"other"} | ${p.duration_minutes||"?"} min`).join(`
`):"(no blocks defined)",b=`Suggest exactly 3 short, evocative names for a rehearsal schedule. Reply with only the 3 names, one per line — no numbers, no bullet points, no explanations.

Production: ${(n==null?void 0:n.name)??"Unknown"}
Date: ${t.date??"not set"}
Blocks:
${d}`;try{const p=await fetch("https://router.huggingface.co/featherless-ai/v1/chat/completions",{method:"POST",headers:{Authorization:`Bearer ${l}`,"Content-Type":"application/json"},body:JSON.stringify({model:"swiss-ai/Apertus-8B-Instruct-2509",messages:[{role:"system",content:"You are a concise assistant helping a theater director name rehearsal schedules."},{role:"user",content:b}],max_tokens:80,temperature:.85})});if(!p.ok){const c=await p.text();a.textContent=`API error ${p.status}: ${c.slice(0,120)}`,i.disabled=!1;return}const T=await p.json(),U=(j=(F=(R=T==null?void 0:T.choices)==null?void 0:R[0])==null?void 0:F.message)==null?void 0:j.content;if(!U){a.textContent="No response from Apertus.",i.disabled=!1;return}const W=U.split(`
`).map(c=>c.replace(/^[\d.\-*•"]+\s*/,"").replace(/["]+$/,"").trim()).filter(c=>c.length>2&&c.length<80).slice(0,3);if(!W.length){a.textContent="Could not parse suggestions. Try again.",i.disabled=!1;return}a.textContent="Pick one to rename this schedule:",s.innerHTML=W.map(c=>`<button class="ai-suggestion-pill" data-name="${S(c)}">${h(c)}</button>`).join(""),s.querySelectorAll(".ai-suggestion-pill").forEach(c=>{c.addEventListener("click",()=>{q(e,{name:c.dataset.name,date:t.date}),k(e)})})}catch(p){a.textContent=`Error: ${p.message}`}finally{i.disabled=!1}}function je(e){return{blocking:"badge-blocking","run-through":"badge-run-through","table work":"badge-table-work",tech:"badge-tech",other:"badge-other"}[e]??"badge-other"}function Ue(e){if(!e)return"";const[t,n,o]=e.split("-");return new Date(+t,+n-1,+o).toLocaleDateString("en-US",{weekday:"long",year:"numeric",month:"long",day:"numeric"})}function h(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function S(e){return String(e??"").replace(/"/g,"&quot;")}C("/",()=>ve());C("/production/:id",({id:e})=>we(e));C("/schedule/:id",({id:e})=>Me(e));ae(()=>g("/"));se();sessionStorage.getItem("tour_seen")||setTimeout(X,400);
